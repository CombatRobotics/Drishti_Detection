...
def metallic_tone(gray, strength):
    if strength <= 0:
        return gray
    gray_f = gray.astype(np.float32)
    mid = 128.0
    out = mid + (gray_f - mid) * (1.0 - strength)
    return np.clip(out, 0, 255).astype(np.uint8)
...
cv2.createTrackbar("Metallic x100", "Controls", 30, 100, lambda x: None)
...
while True:
    ...
    metallic = cv2.getTrackbarPos("Metallic x100", "Controls") / 100.0
    ...
    destriped = vertical_destripe(gray, destripe_strength, mask=non_roi_mask, smooth_sigma=stripe_smooth)

    # ---- Illumination correction ----
    flat = flat_field(destriped, illum_sigma, p_low, p_high, mask_for_stats=roi_mask)

    # ---- ROI enhancement pipeline ----
    roi_proc = flat.copy()
    if roi_nlm_h > 0:
    ...
    roi_proc = metallic_tone(roi_proc, metallic)

    # ---- Background (keep original contrast) ----
    bg_proc = gray.copy()
    if bg_blur > 0:
    ...
    if key == 27 or key == ord('q'):
        break
    if key == ord('s'):
        cv2.imwrite("clahe.png", output)
        print("Saved clahe.png")
